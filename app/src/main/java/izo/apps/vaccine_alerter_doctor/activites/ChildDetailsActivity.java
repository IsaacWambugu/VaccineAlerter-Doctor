...
import izo.apps.vaccine_alerter_doctor.adapters.GuardianAdapter;
import izo.apps.vaccine_alerter_doctor.data.Const;
import izo.apps.vaccine_alerter_doctor.interfaces.LoadContentListener;
import izo.apps.vaccine_alerter_doctor.interfaces.MultiSpinnerListener;
import izo.apps.vaccine_alerter_doctor.interfaces.UploadContentListener;
import izo.apps.vaccine_alerter_doctor.models.GuardianModel;
import izo.apps.vaccine_alerter_doctor.network.NetWorker;
import izo.apps.vaccine_alerter_doctor.network.Mtandao;
import izo.apps.vaccine_alerter_doctor.util.MultiSpinner;
...
public class ChildDetailsActivity extends AppCompatActivity implements UploadContentListener, LoadContentListener {
    ...
    private Boolean isAddVaccine = false;
    ...
    private ArrayList<GuardianModel> extractJSONResponse(JSONObject json) {
        ...
        try {
            ...
            resetImage(atBirthAlert);
            resetImage(sixWAlert);
            resetImage(fourteenWAlert);
            resetImage(sixMAlert);
            resetImage(nineMAlert);
            ...
            public void onItemsSelected(boolean[] selected) {
                ...
                if (!(positions.size() == 0)) {
                    ...
                    for (int x : positions) {
                        ...
                        if (count == 0)
                            vaccines = vaccines.concat(Const.VACCINE_LIST[x]);
                        else
                            vaccines = vaccines.concat("#" + Const.VACCINE_LIST[x]);
        ...
        new CountDownTimer(1000, 2000) {
            ...
            public void onFinish() {
                ...
                if (operation == 1) {
                    ...
                    finish();
                }else if(isAddVaccine){

                    alertDialog.dismiss();
                    isAddVaccine = true;

                }else{
                    startActivity(new Intent(ChildDetailsActivity.this, ChildrenListActivity.class));
                    finish();
