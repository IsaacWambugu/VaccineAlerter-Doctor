package izo.apps.vaccine_alerter_doctor.util;
...
public class MultiSpinner extends AppCompatSpinner implements OnMultiChoiceClickListener, OnCancelListener {
    ...
    private String defaultText = "Click Here To Choose Vaccine(s)";
    ...
    public void onCancel(DialogInterface dialog) {
        ...
        if (spinnerText.length() > 2) {
            ...
            spinnerLayout = R.layout.textview_for_spinner;
        } else {
            spinnerText = defaultText;
            spinnerLayout = R.layout.textview_for_spinner_default;
        }

        ArrayAdapter<String> adapter = new ArrayAdapter<>(getContext(), spinnerLayout, new String[]{spinnerText});
        setAdapter(adapter);
    ...
    public void setItems(LinkedHashMap<String, Boolean> items, MultiSpinnerListener listener) {
        ...
        ArrayAdapter<String> adapter = new ArrayAdapter<>(getContext(), spinnerLayout, new String[]{defaultText});
        setAdapter(adapter);
        // Set Spinner Text
        onCancel(null);
    }

    public void displayItems(List<String> vaccineList){

        this.items = vaccineList;
        StringBuilder spinnerBuffer = new StringBuilder();
        for (int i = 0; i < items.size(); i++) {
            if (selected[i]) {
                spinnerBuffer.append(items.get(i));
